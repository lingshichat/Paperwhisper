...
def safe_archive_paths_to_add(repo_root: Path, task_name: str | None = None) -> list[str]:
    """Return paths to stage after `task.py archive`.

    Limited to the archive subtree (where the freshly-moved task lives),
    active task directories, and (when it was tracked before the move) the
    archived task's former active-path directory so the deletion is staged in
    the same commit. We never stage the whole `.trellis/` tree.
    """
    paths: list[str] = []
    tasks_dir = repo_root / DIR_WORKFLOW / DIR_TASKS
    if tasks_dir.is_dir():
        ...
        if archive_dir.is_dir():
            paths.append(f"{DIR_WORKFLOW}/{DIR_TASKS}/{DIR_ARCHIVE}")
        if task_name:
            # After the move, the source directory no longer exists. Only pass
            # that path to `git add` if Git already tracks files below it;
            # otherwise `git add -- <nonexistent>` would fail with a pathspec
            # error for local-only task records.
            source_rel = f"{DIR_WORKFLOW}/{DIR_TASKS}/{Path(task_name).name}"
            _, tracked, _ = run_git(["ls-files", "--", source_rel], cwd=repo_root)
            if tracked.strip():
        ...
        for child in sorted(tasks_dir.iterdir()):
            ...
            if child.name == DIR_ARCHIVE:
                continue
            child_rel = f"{DIR_WORKFLOW}/{DIR_TASKS}/{child.name}"
