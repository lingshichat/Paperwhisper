...
def _detect_git_bash_path() -> str | None:
    """Detect git-bash path on Windows.

    检测顺序:
    1. 通过 shutil.which('bash') 查找 PATH 中的 bash，推算 Git 安装目录
    2. 常见安装路径扫描
    """
    if sys.platform != "win32":
        return None

    # 方法1: 从 PATH 中的 bash.exe 推算
    bash_in_path = shutil.which("bash")
    if bash_in_path:
        bash_path = Path(bash_in_path).resolve()
        # 典型: D:\Softwares\Git\usr\bin\bash.exe → D:\Softwares\Git\bin\bash.exe
        git_root = bash_path.parent.parent
        if git_root.name == "Git" or (git_root.parent / "bin" / "bash.exe").is_file():
            candidate = git_root.parent / "bin" / "bash.exe"
            if candidate.is_file():
                return str(candidate)
        # 也可能直接是 Git\bin\bash.exe
        candidate = bash_path.parent / "bash.exe"
        if "Git" in str(candidate) and candidate.is_file():
            return str(candidate)
    ...
    common_paths = [
        ...
        Path("D:/Softwares/Git/bin/bash.exe"),
...
def main() -> int:
    ...
    # Windows: 确保子进程能找到 git-bash
    # 从 git-bash 内部启动时环境中可能没有 CLAUDE_CODE_GIT_BASH_PATH，
    # 但子进程脱离 git-bash 上下文后 claude CLI 无法自动检测
    if sys.platform == "win32" and not env.get("CLAUDE_CODE_GIT_BASH_PATH"):
        _git_bash = _detect_git_bash_path()
        if _git_bash:
            env["CLAUDE_CODE_GIT_BASH_PATH"] = _git_bash
            log_info(f"Set CLAUDE_CODE_GIT_BASH_PATH={_git_bash}")
        else:
            log_warn("Could not detect git-bash path. Agent may fail to start on Windows.")
