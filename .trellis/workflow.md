...
<!--
  ...
  Editing checklist:
    ...
    - Runtime parsers in this project:
...
For the workflow state machine's runtime behavior in this project, see:

- `.codex/hooks/inject-workflow-state.py` — per-turn breadcrumb parser (reads `workflow.md` tag blocks only, no embedded fallback text)
- `.trellis/scripts/common/workflow_phase.py` — phase/step extractor used by `get_context.py --mode phase`
