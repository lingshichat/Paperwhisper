...
class PostmarkStamp extends StatelessWidget {
  ...
  const PostmarkStamp({
    ...
    required this.date,
    this.size = 100,
    this.color = const Color(0xCCBF360C), // Deep Orange/Red, slightly transparent
  });
...
class _StampPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Top Arc position
    // Center Star
    drawStar(canvas, Offset(center.dx, center.dy - radius * 0.55), size.width * 0.08); // Big
    // Left Star
    drawStar(canvas, Offset(center.dx - radius * 0.4, center.dy - radius * 0.4), size.width * 0.05); // Small
    // Right Star
    drawStar(canvas, Offset(center.dx + radius * 0.4, center.dy - radius * 0.4), size.width * 0.05); // Small
    ...
    textPainter.layout();
    textPainter.paint(canvas, Offset(center.dx - textPainter.width / 2, center.dy - textPainter.height / 2 + 5)); // Slightly adjusted due to stars
    ...
    for (int i = 0; i < 40; i++) {
      ...
      if (_isInsideCircle(dx, dy, center, radius)) {
         canvas.drawCircle(Offset(dx, dy), random.nextDouble() * 1.5, grungePaint);
      }
    }
  }
