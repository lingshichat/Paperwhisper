...
class DiaryProvider with ChangeNotifier {
  ...
  Map<int, String> get bookTitles =>
      _bookTitles; // Keep for backward compatibility if needed, but prefer getters below
  ...
  Future<void> _loadBookMetadata({bool notify = true}) async {
    try {
      ...
      final metaFile = File(
        path.join(_service.dataDir!.path, 'book_metadata.json'),
      );
            ...
            books.forEach((yearStr, val) {
              ...
              if (year != null && val is Map) {
                ...
                if (val['subtitle'] != null) {
                  _bookSubtitles[year] = val['subtitle'];
                }
                ...
                if (val['cover'] != null) {
                  ...
                  final coverFile = File(
                    path.join(_service.dataDir!.path, coverName),
                  );
  ...
  Future<void> _saveBookMetadata() async {
    ...
    final Map<String, dynamic> data = {'books': {}, 'months': _monthTitles};
    ...
    for (var year in allYears) {
      ...
      if (_bookSubtitles.containsKey(year)) {
        bookInfo['subtitle'] = _bookSubtitles[year];
      }
  ...
  Future<void> setBookInfo(
    ...
    if (coverPath != null) {
      await prefs.setString('book_cover_$year', _bookCoverPaths[year]!);
    }
  }

  Future<void> resetBookInfo(
    int year, {
    bool title = false,
    bool subtitle = false,
    bool cover = false,
